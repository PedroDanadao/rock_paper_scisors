...
function get_computer_choice() {
  ...
  var choice_number = Math.floor(Math.random() * 3); // if the number is 0 then return 'rock', if it is 1 'paper' and if it is 2 'scissors'(use a switch case)
...
function play_round() {
  ...
  console.log("Round Start"); // get the player choice from the user using the prompt function and put it in a player_choice variable

  var player_choice = prompt("Choose between rock paper and scissors by typing those options in the prompt"); // get the computer choice by using the function

  var computer_choice = get_computer_choice(); // check if the user typed a valid option
...
function tester() {
  ...
  typing = check_typing("scissors"); // get the result of a few rounds of the game
