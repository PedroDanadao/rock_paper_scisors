...
// make a function to get the computer choice based on a randomly generated 
// number
function get_computer_choice() {
    // generate a number between 0 and 2
    const choice_number = Math.floor(Math.random() * 3)
    // if the number is 0 then return 'rock', if it is 1 'paper' and if it is 2 
    // 'scissors'(use a switch case)
...
function play_round() {
    ...
    // get the player choice from the user using the prompt function and put it 
    // in a player_choice variable
...
// make a function to get the result between two choices of the game. It will 
// return 1 if the first choice wins, -1 if it looses and 0 if they are the same.
// This function is mostly for testing things out
function get_result(player_choice: string, computer_choice: string) {
    // get the strings of the player and the computer and attribute 'rock' to 2,
    // 'paper' to 3 and 'scissors' to 5
    ...
    // check if both numbers are equal, if so then announce it to the user and 
    // return a 'draw' value(0)
    if (player_choice_number === computer_choice_number) {
        console.log(draw_string);
        return 0;
    }

    // check if the double of the first number is less then the second number. 
    // If so then it is the case where the user chose a rock and the computer 
    // chose scissors. In that case, announce it to the user and return the win 
    // value(1)
    ...
    // check the opposite now. If the double of the second number is less that 
    // the first number. If so then announce that the user lost and return the 
    // loose value(0)
    ...
    // check if the first number is more than the second number. If so then 
    // return a win value.
...
// make a function to check if a passed string in lower case is either 'rock', 
// 'paper', or 'scissors'
...
function game() {
    // create a variable(i) to increment the number of matches between the user 
    // and the computer.
    // It will be necessary to use it alongside a while loop to deal with 
    // possible misspells
    // of the user. The variable will only increment when the user types a valid
    // option
    let i = 0;

    // make a variable to record the points of the player, one for the points of
    // the computer and one for the draws
    let player_points = 0;
    ...
    while (i < 5) {
        ...
        // check if the round result is three. If so, then play another round
        // without incrementing i
        if (round_result === 3) {
            continue;
        }

        // increment the player score, the computer score or the draw score 
        // based on the result of the compare function.
        ...
        // check if the current round is the last one. If not, then print that a
        // new match will start
        if (i != 4) {
    ...
    // compare the results between the player and the computer and announce the 
    // winner(if the results are the same then announce a draw)
    let results_string = '';
...
function test_score(score_number_identifier: number) {
    // make three variables for the win, lose and draw results their value will 
    // be based on the passed score_number_identifier after running a few math 
    // expressions
